...
function get_symbol_info(symbol::String, api_key::String, api_secret::String)
    try
        base_url = "https://testnet.binancefuture.com"
        response = HTTP.get("$base_url/fapi/v1/exchangeInfo")
        ...
        return Dict("error" => "Symbol not found")
    catch e
        println("Error getting symbol info: $e")
        return Dict("error" => string(e))
    end
end
...
function cancel_all_orders(symbol::String, api_key::String, api_secret::String)
    try
        ...
        params = Dict("symbol" => symbol, "timestamp" => timestamp)
        ...
        response = HTTP.delete("$base_url/fapi/v1/allOpenOrders?$full_query", headers)
        return JSON3.read(String(response.body))
    catch e
        println("Cancel orders error: $e")
        return Dict("error" => string(e))
    end
end
...
function calculate_order_size(available_balance::Float64, price::Float64, max_capital::Float64, num_levels::Int)
    # Conservative order sizing for testnet
    max_position_value = max_capital / (num_levels * 4)  # More conservative
    size_by_capital = max_position_value / price
    size_by_balance = available_balance * 0.05 / price  # Use only 5% of balance per order
    
    # For ETHUSDT on testnet, typical minimum is 0.001 ETH
    min_size = 0.001
    calculated_size = min(size_by_capital, size_by_balance)
